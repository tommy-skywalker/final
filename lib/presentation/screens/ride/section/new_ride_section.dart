...
class _NewRideSectionState extends State<NewRideSection> {
  ...
  void scrollListener() {
    if (scrollController.position.pixels == scrollController.position.maxScrollExtent) {
      if (Get.find<RideHistoryController>().hasNext()) {
        Get.find<RideHistoryController>().getRideList(AppStatus.RIDE_PENDING.toString());
      }
    }
  }
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((time) {
      controller.initialData(isIntraCity: widget.isInterCity, status: AppStatus.RIDE_PENDING.toString()).then((v) {
        ...
        return RefreshIndicator(
          ...
          child: Padding(
            ...
            child: controller.isLoading
                ? SingleChildScrollView(padding: Dimensions.sectionPadding, child: Column(children: List.generate(10, (index) => const RideShimmer())))
                : controller.isLoading == false && controller.rideList.isEmpty
                    ? const NoDataWidget(text: MyStrings.noRideFound, fromRide: true)
                    : ListView.builder(
